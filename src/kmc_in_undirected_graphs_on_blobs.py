...
classifiers: Mapping[str, ClusteringLabeledClassifier] = {
    'K_means clustering on blobs': KMeansLabeledClassifier(blobs_database)
}
...
k_means_clustering = classifiers['K_means clustering on blobs'].clustering
assert isinstance(k_means_clustering, ArrayBasedClustering)

evaluation_text = get_ar_and_nmi_text(
    classification_evaluations['K_means clustering on blobs']
)
